...
void EXT_FUNC ClientCommand(edict_t *pEntity)
{
    ...
    else if(FStrEq(pcmd, "setspawn"))
    {
      CBaseEntity *spawn = CBaseEntity::Create( "info_player_deathmatch", player->pev->origin, player->pev->angles, player->edict() );
      CONSOLE_ECHO_("Created!\n");
      player->setspawn = true;
    }
    else if(FStrEq(pcmd, "remove_all_spawnpoints"))
    {
	...
	else if (FStrEq(pcmd, "menuselect"))
	{
        ...
        case Menu_CSDM_Equip_Main:
        {
            switch(slot)
            {
                case 1:
                {
                    ShowMenu(player, (MENU_KEY_1 | MENU_KEY_2 | MENU_KEY_3 | MENU_KEY_4 | MENU_KEY_5 | MENU_KEY_6 | MENU_KEY_0), 120, FALSE, menutext);
                    player->m_iMenu = Menu_CSDM_Equip_Pistol;
                    return;
                }
                case 2:
                {
                  if(player->primary == NULL && player->secondary == NULL) return;
                  player->RemoveWeapons();
                  if(player->primary != NULL)
                        player->GiveNamedItem(player->primary);
                   if(player->secondary != NULL)
                        player->GiveNamedItem(player->secondary);
                   if (BuyAmmo(player, PRIMARY_WEAPON_SLOT, true)) while (BuyAmmo(player, PRIMARY_WEAPON_SLOT, false)) ;
                   if (BuyAmmo(player, PISTOL_SLOT, true)) while (BuyAmmo(player, PISTOL_SLOT, false)) ;
                   player->m_iMenu = Menu_OFF;
                   return;
                }
                case 3:
                {
                    ...
                    if (BuyAmmo(player, PRIMARY_WEAPON_SLOT, true)) while (BuyAmmo(player, PRIMARY_WEAPON_SLOT, false)) ;
                    if (BuyAmmo(player, PISTOL_SLOT, true)) while (BuyAmmo(player, PISTOL_SLOT, false)) ;
                      player->AutoEquip = true;
                      player->m_iMenu = Menu_OFF;
                      return;
                }
            }
        }
        case Menu_CSDM_Equip_Page1:
        {
            switch(slot)
            {
            case 1:
            ...
            {
                player->RemoveWeapons();
                player->GiveNamedItem("weapon_aug");
                player->GiveNamedItem(player->secondary);
                player->primary = "weapon_aug";
                if (BuyAmmo(player, PRIMARY_WEAPON_SLOT, true)) while (BuyAmmo(player, PRIMARY_WEAPON_SLOT, false)) ;
                player->m_iMenu = Menu_OFF;
                return;
            }
            ...
            {
                player->RemoveWeapons();
                player->GiveNamedItem("weapon_sg552");
                player->GiveNamedItem(player->secondary);
                player->primary = "weapon_sg552";
                if (BuyAmmo(player, PRIMARY_WEAPON_SLOT, true)) while (BuyAmmo(player, PRIMARY_WEAPON_SLOT, false)) ;
                player->m_iMenu = Menu_OFF;
                return;
            }
            ...
            {
                player->RemoveWeapons();
                player->GiveNamedItem("weapon_famas");
                player->GiveNamedItem(player->secondary);
                player->primary = "weapon_famas";
                if (BuyAmmo(player, PRIMARY_WEAPON_SLOT, true)) while (BuyAmmo(player, PRIMARY_WEAPON_SLOT, false)) ;
                player->m_iMenu = Menu_OFF;
                return;
            }
            ...
            {
                player->RemoveWeapons();
                player->GiveNamedItem("weapon_galil");
                player->GiveNamedItem(player->secondary);
                player->primary = "weapon_galil";
                if (BuyAmmo(player, PRIMARY_WEAPON_SLOT, true)) while (BuyAmmo(player, PRIMARY_WEAPON_SLOT, false)) ;
                player->m_iMenu = Menu_OFF;
                return;
            }
            case 9:
                ShowMenu(player, (MENU_KEY_1 | MENU_KEY_2 | MENU_KEY_3 | MENU_KEY_4 | MENU_KEY_5 | MENU_KEY_6 | MENU_KEY_9 | MENU_KEY_0), 120, FALSE, "CSDM: Equip \n\n1. TMP\n 2. MAC10\n 3. UMP45\n 4. MP5\n 5. P90\n6. M249\n9. Next\n\n0. Quit\n");
                player->m_iMenu = Menu_CSDM_Equip_Page2;
                return;
            case 0:
                player->m_iMenu = Menu_OFF;
                return;
        ...
        case Menu_CSDM_Equip_Page2:
        {
            switch(slot)
            {
            case 1:
                player->RemoveWeapons();
                player->GiveNamedItem("weapon_tmp");
                player->GiveNamedItem(player->secondary);
                player->primary = "weapon_tmp";
                if (BuyAmmo(player, PRIMARY_WEAPON_SLOT, true)) while (BuyAmmo(player, PRIMARY_WEAPON_SLOT, false)) ;
                player->m_iMenu = Menu_OFF;
                return;
            case 2:
                player->RemoveWeapons();
                player->GiveNamedItem("weapon_mac10");
                player->GiveNamedItem(player->secondary);
                player->primary = "weapon_mac10";
                if (BuyAmmo(player, PRIMARY_WEAPON_SLOT, true)) while (BuyAmmo(player, PRIMARY_WEAPON_SLOT, false)) ;
                player->m_iMenu = Menu_OFF;
                return;
            case 3:
                ...
                player->GiveNamedItem(player->secondary);
                player->primary = "weapon_ump45";
                if (BuyAmmo(player, PRIMARY_WEAPON_SLOT, true)) while (BuyAmmo(player, PRIMARY_WEAPON_SLOT, false)) ;
                player->m_iMenu = Menu_OFF;
                return;
            case 4:
                player->RemoveWeapons();
                player->GiveNamedItem("weapon_mp5navy");
                player->GiveNamedItem(player->secondary);
                player->primary = "weapon_mp5navy";
                if (BuyAmmo(player, PRIMARY_WEAPON_SLOT, true)) while (BuyAmmo(player, PRIMARY_WEAPON_SLOT, false)) ;
                player->m_iMenu = Menu_OFF;
                return;
            case 5:
                ...
                player->GiveNamedItem(player->secondary);
                player->primary = "weapon_p90";
                if (BuyAmmo(player, PRIMARY_WEAPON_SLOT, true)) while (BuyAmmo(player, PRIMARY_WEAPON_SLOT, false)) ;
                player->m_iMenu = Menu_OFF;
                return;
            case 6:
                player->RemoveWeapons();
                player->GiveNamedItem("weapon_m249");
                player->GiveNamedItem(player->secondary);
                player->primary = "weapon_m249";
                if (BuyAmmo(player, PRIMARY_WEAPON_SLOT, true)) while (BuyAmmo(player, PRIMARY_WEAPON_SLOT, false)) ;
                player->m_iMenu = Menu_OFF;
                return;
            case 9:
                ShowMenu(player, (MENU_KEY_1 | MENU_KEY_2 | MENU_KEY_3 | MENU_KEY_4 | MENU_KEY_5 | MENU_KEY_6 | MENU_KEY_0), 120, FALSE, "CSDM: Equip \n\n1. M3\n2. XM1014\n3. SCOUT\n4. AWP\n5. SG550\n6. G3SG1\n\n0. Quit\n");
                player->m_iMenu = Menu_CSDM_Equip_Page3;
                return;
            case 0:
                player->m_iMenu = Menu_OFF;
                return;
        ...
        case Menu_CSDM_Equip_Page3:
        {
            switch(slot)
            {
            case 1:
                player->RemoveWeapons();
                player->GiveNamedItem("weapon_m3");
                player->GiveNamedItem(player->secondary);
                player->primary = "weapon_m3";
                if (BuyAmmo(player, PRIMARY_WEAPON_SLOT, true)) while (BuyAmmo(player, PRIMARY_WEAPON_SLOT, false)) ;
                player->m_iMenu = Menu_OFF;
                return;
            case 2:
                ...
                player->GiveNamedItem(player->secondary);
                player->primary = "weapon_xm1014";
                if (BuyAmmo(player, PRIMARY_WEAPON_SLOT, true)) while (BuyAmmo(player, PRIMARY_WEAPON_SLOT, false)) ;
                player->m_iMenu = Menu_OFF;
                return;
            case 3:
                player->RemoveWeapons();
                player->GiveNamedItem("weapon_scout");
                player->GiveNamedItem(player->secondary);
                player->primary = "weapon_scout";
                if (BuyAmmo(player, PRIMARY_WEAPON_SLOT, true)) while (BuyAmmo(player, PRIMARY_WEAPON_SLOT, false)) ;
                player->m_iMenu = Menu_OFF;
                return;
            case 4:
                ...
                player->GiveNamedItem(player->secondary);
                player->primary = "weapon_awp";
                if (BuyAmmo(player, PRIMARY_WEAPON_SLOT, true)) while (BuyAmmo(player, PRIMARY_WEAPON_SLOT, false)) ;
                player->m_iMenu = Menu_OFF;
                return;
            case 5:
                player->RemoveWeapons();
                player->GiveNamedItem("weapon_sg550");
                player->GiveNamedItem(player->secondary);
                player->primary = "weapon_sg550";
                if (BuyAmmo(player, PRIMARY_WEAPON_SLOT, true)) while (BuyAmmo(player, PRIMARY_WEAPON_SLOT, false)) ;
                player->m_iMenu = Menu_OFF;
                return;
            case 6:
                player->RemoveWeapons();
                player->GiveNamedItem("weapon_g3sg1");
                player->GiveNamedItem(player->secondary);
                player->primary = "weapon_g3sg1";
                if (BuyAmmo(player, PRIMARY_WEAPON_SLOT, true)) while (BuyAmmo(player, PRIMARY_WEAPON_SLOT, false)) ;
                player->m_iMenu = Menu_OFF;
                return;
            case 0:
        ...
        case Menu_CSDM_Equip_Pistol:
        {
            switch(slot)
            {
            case 1:
                player->RemoveWeapons();
                player->GiveNamedItem("weapon_glock18");
                player->secondary = "weapon_glock18";
                if (BuyAmmo(player, PISTOL_SLOT, true)) while (BuyAmmo(player, PISTOL_SLOT, false)) ;
                ShowMenu(player, (MENU_KEY_1 | MENU_KEY_2 | MENU_KEY_3 | MENU_KEY_4 | MENU_KEY_5 | MENU_KEY_6 | MENU_KEY_9 | MENU_KEY_0), 120, FALSE, "CSDM: Equip \n\n1. M4A1\n2. AK47\n3. AUG\n4. SG552\n5. FAMAS\n6. GALIL\n9. Next\n\n0. Quit");
                player->m_iMenu = Menu_CSDM_Equip_Page1;
                return;
            case 2:
                player->RemoveWeapons();
                player->GiveNamedItem("weapon_usp");
                player->secondary = "weapon_usp";
                if (BuyAmmo(player, PISTOL_SLOT, true)) while (BuyAmmo(player, PISTOL_SLOT, false)) ;
                ShowMenu(player, (MENU_KEY_1 | MENU_KEY_2 | MENU_KEY_3 | MENU_KEY_4 | MENU_KEY_5 | MENU_KEY_6 | MENU_KEY_9 | MENU_KEY_0), 120, FALSE, "CSDM: Equip \n\n1. M4A1\n2. AK47\n3. AUG\n4. SG552\n5. FAMAS\n6. GALIL\n9. Next\n\n0. Quit");
                player->m_iMenu = Menu_CSDM_Equip_Page1;
                return;
            case 3:
                player->RemoveWeapons();
                player->GiveNamedItem("weapon_p228");
                player->secondary = "weapon_p228";
                if (BuyAmmo(player, PISTOL_SLOT, true)) while (BuyAmmo(player, PISTOL_SLOT, false)) ;
                ShowMenu(player, (MENU_KEY_1 | MENU_KEY_2 | MENU_KEY_3 | MENU_KEY_4 | MENU_KEY_5 | MENU_KEY_6 | MENU_KEY_9 | MENU_KEY_0), 120, FALSE, "CSDM: Equip \n\n1. M4A1\n2. AK47\n3. AUG\n4. SG552\n5. FAMAS\n6. GALIL\n9. Next\n\n0. Quit");
                player->m_iMenu = Menu_CSDM_Equip_Page1;
                return;
            case 4:
                ...
                player->secondary = "weapon_deagle";
                if (BuyAmmo(player, PISTOL_SLOT, true)) while (BuyAmmo(player, PISTOL_SLOT, false)) ;
                ShowMenu(player, (MENU_KEY_1 | MENU_KEY_2 | MENU_KEY_3 | MENU_KEY_4 | MENU_KEY_5 | MENU_KEY_6 | MENU_KEY_9 | MENU_KEY_0), 120, FALSE, "CSDM: Equip \n\n1. M4A1\n2. AK47\n3. AUG\n4. SG552\n5. FAMAS\n6. GALIL\n9. Next\n\n0. Quit");
                player->m_iMenu = Menu_CSDM_Equip_Page1;
                return;
            case 5:
                ...
                player->secondary = "weapon_fiveseven";
                if (BuyAmmo(player, PISTOL_SLOT, true)) while (BuyAmmo(player, PISTOL_SLOT, false)) ;
                ShowMenu(player, (MENU_KEY_1 | MENU_KEY_2 | MENU_KEY_3 | MENU_KEY_4 | MENU_KEY_5 | MENU_KEY_6 | MENU_KEY_9 | MENU_KEY_0), 120, FALSE, "CSDM: Equip \n\n1. M4A1\n2. AK47\n3. AUG\n4. SG552\n5. FAMAS\n6. GALIL\n9. Next\n\n0. Quit");
                player->m_iMenu = Menu_CSDM_Equip_Page1;
                return;
