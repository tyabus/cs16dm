...
CBasePlayer *CCSBot::FindMostDangerousThreat()
{
...

	{
		for (i = 1; i <= gpGlobals->maxClients; ++i)
		{
			...
			// is it an enemy?
			if (player->m_iTeam == m_iTeam && (CVAR_GET_FLOAT("team_deathmatch") == 1))
			{
				TraceResult result;
...
void CCSBot::UpdateReactionQueue()
{
	...
	float maxReactionTime = (MAX_ENEMY_QUEUE * g_flBotFullThinkInterval) - 1.0f;
	if (reactionTime > maxReactionTime)
		reactionTime = maxReactionTime;

	// "rewind" time back to our reaction time
	int reactionTimeSteps = (int)((reactionTime / g_flBotFullThinkInterval) + 0.05f);
