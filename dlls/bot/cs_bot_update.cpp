...
void CCSBot::Update()
{
    ...
    if(CVAR_GET_FLOAT("bots_only_strafe") > 0)
    {
        StrafeLeft();
        StrafeRight();
        return;
	...
	if (cv_bot_show_nav.value > 0.0f && m_lastKnownArea != NULL)
	{
        ...
        if((target = UTIL_FindEntityByClassname(0, "way_start")) != NULL)
        {
			Run();
			if(next == false)
			{			
				MoveTo(&target->pev->origin, FASTEST_ROUTE);
				if((target->pev->origin - pev->origin).Length() < 80)
				{
					next = true;
				}
			}
			...
			if(next == true)
			{
			   if((target = UTIL_FindEntityByClassname(0, "way_end")) != NULL)
			    {
				   MoveTo(&target->pev->origin, FASTEST_ROUTE);
					if((target->pev->origin - pev->origin).Length() < 80)
						next = false;
