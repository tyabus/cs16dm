...
BOOL CHalfLifeMultiplay::FPlayerCanTakeDamage(CBasePlayer *pPlayer, CBaseEntity *pAttacker)
{
	...
	if (CVAR_GET_FLOAT("mp_friendlyfire") != 0 || pAttacker == pPlayer || (CVAR_GET_FLOAT("team_deathmatch") == 0))
	{
		return TRUE;
	}
...
Vector SetColor()
{
    if(FStrEq(CVAR_GET_STRING("kill_color"), "red"))
        return Vector(200, 0, 0);
    else if(FStrEq(CVAR_GET_STRING("kill_color"), "blue"))
        return Vector(0, 0, 200);
    else if(FStrEq(CVAR_GET_STRING("kill_color"), "green"))
        return Vector(0, 200, 0);
    else if(FStrEq(CVAR_GET_STRING("kill_color"), "purple"))
        return Vector(128, 0 , 128);
    else if(FStrEq(CVAR_GET_STRING("kill_color"), "yellow"))
        return Vector(255, 255, 0);
    else if(FStrEq(CVAR_GET_STRING("kill_color"), "orange"))
        return Vector(255, 128, 0);
    else if(FStrEq(CVAR_GET_STRING("kill_color"), "grey"))
        return Vector(80, 80, 80);
    else if(FStrEq(CVAR_GET_STRING("kill_color"), "random"))
        return Vector(RANDOM_LONG(0, 255), RANDOM_LONG(0, 255), RANDOM_LONG(0, 255));
    else
        CVAR_SET_STRING("kill_color", "none");
...
void CHalfLifeMultiplay::PlayerKilled(CBasePlayer *pVictim, entvars_t *pKiller, entvars_t *pInflictor)
{
    ...
    if((spawn = UTIL_FindEntityByClassname(0, "info_player_deathmatch")) == NULL)
    {
        CONSOLE_ECHO_("Warning! Spawnpoint not found!\n");
        CONSOLE_ECHO_("Creating random point...\n");

        float cvar = CVAR_GET_FLOAT("rand_spawn");

        if(cvar <= 0)
            CVAR_SET_FLOAT("rand_spawn", 1);

        spawn = CBaseEntity::Create("info_player_deathmatch", g_pGameRules->FixSpawn(Vector(RANDOM_FLOAT(-4000, 4000),RANDOM_FLOAT(-4000, 4000), 1), FALSE), g_vecZero);

        if(cvar <= 0)
           CVAR_SET_FLOAT("rand_spawn", 0);
    }
	...
	else if (peKiller && peKiller->IsPlayer())
	{
        ...
        if(!FStrEq(CVAR_GET_STRING("kill_color"), "none"))
            UTIL_ScreenFade(killer, SetColor(), 0.5, 0.3, 120, 0);

        killer->TakeHealth(10, NULL);
        killer->m_iKills++;
...
void CHalfLifeMultiplay::DeathNotice(CBasePlayer *pVictim, entvars_t *pKiller, entvars_t *pevInflictor)
{
    ...
    if(CVAR_GET_FLOAT("quake_sounds") > 0)
    {
        CBasePlayer *pPlayer = dynamic_cast<CBasePlayer *>(CBaseEntity::Instance(pKiller));

        if( !m_fFirstBlood )
        {
            EMIT_SOUND(ENT(pKiller), CHAN_ITEM, "csdm/firstblood.wav", 1, ATTN_NONE);
            m_fFirstBlood = true;
        }

        else if( pVictim->m_bHeadshotKilled )
            EMIT_SOUND(ENT(pKiller), CHAN_ITEM, "csdm/headshot.wav", 1, ATTN_NONE);

        else if(  pPlayer->m_iKills == 2 )
            EMIT_SOUND(ENT(pKiller), CHAN_ITEM, "csdm/dominating.wav", 1, ATTN_NONE);

        else if(  pPlayer->m_iKills == 3 )
            EMIT_SOUND(ENT(pKiller), CHAN_ITEM, "csdm/hattrick.wav", 1, ATTN_NONE);

        else if(  pPlayer->m_iKills == 4 )
            EMIT_SOUND(ENT(pKiller), CHAN_ITEM, "csdm/godlike.wav", 1, ATTN_NONE);

        else if(  pPlayer->m_iKills == 5 )
            EMIT_SOUND(ENT(pKiller), CHAN_ITEM, "csdm/ultrakill.wav", 1, ATTN_NONE);

        else if( pPlayer->m_iKills == 6 )
            EMIT_SOUND(ENT(pKiller), CHAN_ITEM, "csdm/monsterkill.wav", 1, ATTN_NONE);
    }
