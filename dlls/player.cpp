...
CBasePlayer *CBasePlayer::GetNextRadioRecipient(CBasePlayer *pStartPlayer)
{
	...
	while ((pEntity = UTIL_FindEntityByClassname(pEntity, "player")) != NULL)
	{
		...
		if (pEntity->IsPlayer())
		{
			...
			if (pPlayer && pPlayer->m_iTeam == m_iTeam && (CVAR_GET_FLOAT("team_deathmatch") == 1))
				bSend = TRUE;
		}
		else if (pPlayer)
		{
			...
			if (pTarget && pTarget->m_iTeam == m_iTeam &&  (CVAR_GET_FLOAT("team_deathmatch") == 1))
			{
				bSend = TRUE;
			}
		}
...
void CBasePlayer::Radio(const char *msg_id, const char *msg_verbose, short pitch, bool showIcon)
{
	...
	while ((pEntity = UTIL_FindEntityByClassname(pEntity, "player")) != NULL)
	{
		...
		if (pPlayer->IsPlayer())
		{
			...
			if (pPlayer->m_iTeam == m_iTeam &&  (CVAR_GET_FLOAT("team_deathmatch") == 1))
				bSend = TRUE;
		}
...
void CBasePlayer::TraceAttack(entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType)
{
	...
	else
	{
		...
		case HITGROUP_CHEST:
		{
			if(CVAR_GET_FLOAT("headshot_mode") != 0)
				return;
			flDamage *= 1;
    ...
    textParms.fadeoutTime = 2.0f;
    textParms.holdTime = 2.0f;
    textParms.fxTime = 0.0f;

    textParms.channel = 0;

    UTIL_HudMessage(pAttacker, textParms, dmg);

   // UTIL_ShowMessage(dmg, pAttacker);
...
int CBasePlayer::TakeDamage(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType)
{
	...
	//	UTIL_ShowMessage("LOL", pAttacker, false);
		return fTookDamage;
	}
...
void CBasePlayer::RemoveWeapons()
{
    ...
    if(!IsBot())
    {
        if(!AutoEquip)
        {
            ...
            case CT:
                ...
                GiveAmmo(24, "45acp", MAX_AMMO_45ACP);
        ...
        {
            if(primary != NULL)
                GiveNamedItem(primary);
            if(secondary != NULL)
                GiveNamedItem((secondary));
        }
    }
    else
    {
        GiveNamedItem(primaryweaponlist[RANDOM_LONG(0, 17)]);
        GiveNamedItem(pistollist[RANDOM_LONG(0, 5)]);
    }

    if(m_rgpPlayerItems[ PRIMARY_WEAPON_SLOT ] != NULL)
    {
        if (BuyAmmo(this, PRIMARY_WEAPON_SLOT, true)) while (BuyAmmo(this, PRIMARY_WEAPON_SLOT, false)) ;
    }

    if (m_rgpPlayerItems[ PISTOL_SLOT ] != NULL)
    {
        if (BuyAmmo(this, PISTOL_SLOT, true)) while (BuyAmmo(this, PISTOL_SLOT, false)) ;
    }
...
{
	m_TimeWeaponRemove = gpGlobals->time + 4;

    if(m_iMenu != Menu_OFF && m_iMenu != Menu_ChooseAppearance)
    {
        ShowMenu(this, (MENU_KEY_0), 0, 0, "O-ops...");
        m_iMenu = Menu_OFF;
    }
...
void CBasePlayer::RoundRespawn()
{
	...
	pev->armorvalue = 100;
	m_GodTime = gpGlobals->time + CVAR_GET_FLOAT("god_time");
...
void CBasePlayer::PreThink()
{
    ...
    if ( pev->button & IN_JUMP || m_afButtonPressed & IN_JUMP)
	{
        ...
        {
            m_speed = pev->velocity.y;
            for(int i = 0;i<50;i++)
            {
                if(pev->flags & FL_ONGROUND && IsAlive())
                    pev->velocity.z += 5;
                    pev->velocity.y = m_speed;
                pev->button &= ~IN_JUMP;
                SetAnimation( PLAYER_JUMP );
            //pev->velocity.z -= 20;
            }
...
CTSpawn:
    ...
    {
        if((ent = UTIL_FindEntityByClassname(ent, "info_player_deathmatch")) != NULL)
...
edict_t *hEntSelectSpawnPoint( CBaseEntity *pPlayer )
{
    CBaseEntity *pSpot;
    edict_t		*player;
    CBasePlayer *p;
...
// choose a info_player_deathmatch point
	...
	else if ( g_pGameRules->IsDeathmatch() )
	{
            ...
            for ( int i = RANDOM_LONG(1,15); i > 0; i-- )
...
void CBasePlayer::Spawn()
{
    ...
    if(!IsBot() && !AutoEquip)
    {
        ShowMenu(this, (MENU_KEY_1 | MENU_KEY_2 | MENU_KEY_3), 120, FALSE, "CSDM: Equip \n\n1. New Weapons\n2. Previous Setup\n 3. Previous Setup + Dont show again\n");
        m_iMenu = Menu_CSDM_Equip_Main;
    }
}
...
int CBasePlayer::Restore(CRestore &restore)
{
	...
	if (!pSaveData->fUseLandmark)
	{
		...
		if(CVAR_GET_FLOAT("team_deathmatch") <= 0)
			pentSpawnSpot = hEntSelectSpawnPoint(this);
		else
			pentSpawnSpot = EntSelectSpawnPoint(this);
	...
	if (tr.flFraction != 1.0f)
	{
		if (!FNullEnt(tr.pHit))
		{
			...
			if (gpGlobals->time >= m_blindUntilTime && isVisiblePlayer)
			{
				...
				if (pTarget->m_iTeam == m_iTeam && (CVAR_GET_FLOAT("team_deathmatch") == 1)|| IsObserver())
				{
...
bool CBasePlayer::NeedsDefuseKit()
{
	...
	return false;//(mpRules->m_bMapHasBombTarget);
}
...
void CBasePlayer::UpdateLocation(bool forceUpdate)
{
	...
	for (int i = 1; i <= gpGlobals->maxClients; ++i)
	{
		...
		if (player->m_iTeam == m_iTeam && (CVAR_GET_FLOAT("team_deathmatch") == 1)|| player->m_iTeam == SPECTATOR)
		{
